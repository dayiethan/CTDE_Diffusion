...
model1 = ImitationNet(input_size=2, hidden_size=256, output_size=2)
model2 = ImitationNet(input_size=2, hidden_size=256, output_size=2)
...
save_path1 = "trained_models/bc/retry1.pth"
save_path2 = "trained_models/bc/retry2.pth"
...
for i in range(100):
    ...
    np.save(f"sampled_trajs/bc_restore/mpc_traj1_{i}.npy", np.array(traj1))
    generated_trajectories2.append(np.array(traj2))
    np.save(f"sampled_trajs/bc_restore/mpc_traj2_{i}.npy", np.array(traj2))
