...
import pdb
import csv
from mpc_util import mpc_plan_multi
...
# Training
action_cond_ode = Conditional_ODE(env, [8, 8], sig.tolist(), device=device, N=100, n_models = 2, **model_size)
...
for i in range(10):
    ...
    plt.savefig("figs/mpc/collision_conditioning/mpc_traj_%s.png" % i)
    plt.show()
    ...
    ani.save("figs/mpc/collision_conditioning/mpc_ani_%s.mp4" % i, writer="ffmpeg", fps=12)
    plt.close()
