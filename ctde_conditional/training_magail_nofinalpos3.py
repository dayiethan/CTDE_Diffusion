...
def build_sa(expert, other):
    ...
    for i in range(len(expert)):
        ...
        for t in range(len(traj)-1):
            s = np.hstack([traj[t], other_traj[t]])  # (4,)
            a = traj[t+1]                   # (2,)
...
# 8) Save models
save_path_G1 = "trained_models/magail/G1_nofinalpos_big_200epochs.pth"
save_path_G2 = "trained_models/magail/G2_nofinalpos_big_200epochs.pth"
save_path_D  = "trained_models/magail/D_nofinalpos_big_200epochs.pth"
...
for i in range(num_samples):
    ...
    np.save(f"sampled_trajs/magail_nofinalpos_200epochs/vary_init/mpc_traj1_{i}.npy", traj1)
    traj2 = sample_trajectory(G2, init2, init1, T)
    np.save(f"sampled_trajs/magail_nofinalpos_200epochs/vary_init/mpc_traj2_{i}.npy", traj2)
...
for i in range(N):
    np.save(f"sampled_trajs/magail_nofinalpos_200epochs/static_init/mpc_traj1_{i}.npy", rollouts1[i])
    np.save(f"sampled_trajs/magail_nofinalpos_200epochs/static_init/mpc_traj2_{i}.npy", rollouts2[i])
