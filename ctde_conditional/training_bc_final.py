...
for i in range(len(expert_data_1)):
    for j in range(len(expert_data_1[i]) - 1):
        X_train1.append(np.hstack([expert_data_1[i][j], expert_data_2[i][j]]))  # Current state + other agent's current position
...
for i in range(len(expert_data_2)):
    for j in range(len(expert_data_2[i]) - 1):
        X_train2.append(np.hstack([expert_data_2[i][j], expert_data_1[i][j]]))  # Current state + other agent's current position
