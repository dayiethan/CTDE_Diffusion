...
for i in range(len(expert_data_1)):
    for j in range(len(expert_data_1[i]) - 1):
        X_train1.append(np.hstack([expert_data_1[i][j]]))  # Current state
...
for i in range(len(expert_data_2)):
    for j in range(len(expert_data_2[i]) - 1):
        X_train2.append(np.hstack([expert_data_2[i][j]]))  # Current state
...
save_path1 = "trained_models/bc/blah1.pth"
save_path2 = "trained_models/bc/blah2.pth"
