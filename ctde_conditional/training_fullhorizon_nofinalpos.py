...
# action_cond_ode.train([actions1, actions2], [attr1, attr2], int(5*n_gradient_steps), batch_size, extra="_T100_nofinalpos")
# action_cond_ode.save(extra="_T100_nofinalpos")
action_cond_ode.load(extra="_T100_nofinalpos")
...
for i in range(100):
    ...
    cond1 = np.hstack([initial1, initial2])
    cond2 = np.hstack([initial2, initial1])
    ...
    np.save("sampled_trajs/T100_nofinalpos/traj1_%s.npy" % i, sampled1)
    np.save("sampled_trajs/T100_nofinalpos/traj2_%s.npy" % i, sampled2)
