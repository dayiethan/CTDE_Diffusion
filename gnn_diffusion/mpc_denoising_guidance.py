...
from conditional_Action_DiT_denoising_guidance import Conditional_ODE
...
def mpc_plan_multi_true(ode_model, env, initial_states, fixed_goals, segment_length=10, total_steps=100, n_implement=5, mean = 0, std = 1):
    """
    ...
    for step in range(n_steps):
        ...
        for i in range(n_agents):
            ...
            sampled = ode_model.sample_denoising_guidance_2(
                ...
                n_samples=1,
                w=1.,
                model_index=i, mean = mean, std = std
            )
...
for i in range(10):
    ...
    planned_trajs = mpc_plan_multi_true(action_cond_ode, env, [initial2, initial1], [final2, final1], segment_length=H, total_steps=T, n_implement = 5, mean = mean, std = std)
    ...
    save_folder = "data/mpc_H_25_I_5_denoising_guidance"

    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    if not os.path.exists("figs/mpc_noise_H_25_I_5_denoising_guidance"):
        os.makedirs("figs/mpc_noise_H_25_I_5_denoising_guidance")
    ...
    plt.ylim(-7, 7)
    plt.xlim(-1,21)
    plt.plot(planned_traj1[:, 0], planned_traj1[:, 1], 'b.-', label="Agent 1")
    plt.plot(planned_traj2[:, 0], planned_traj2[:, 1], 'o-', color='orange', label="Agent 2")
    plt.legend()
    ox, oy, r = obstacle
