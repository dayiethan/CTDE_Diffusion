...
origin_2 = np.array([20.0,0.0])
# origin_3 = np.array([10.0*math.cos(math.pi/3),10*math.sin(math.pi/3)])

destination_1 = np.array([20.0, 0.0])
destination_2 = np.array([0.0, 0.0])
...
trainer.save(path='trained_models/mlp_gnn_H_10_decentralized_2_agents.pt')

# trainer.load(path='trained_models/gnn_baseline_H_10_decentralized_2_agents.pt')
...
for i in range(10):
    ...
    sampled1 = trainer.sample_trajectory([attr_t1, attr_t2], agent=0)[0,:,:]
    sampled2 = trainer.sample_trajectory([attr_t1, attr_t2], agent=1)[0,:,:]
    ...
    if not os.path.exists("figs/2agents_gnn_H_10"):
        os.makedirs("figs/2agents_gnn_H_10")
    plt.savefig("figs/2agents_gnn_H_10/plot%s.png" % (i))