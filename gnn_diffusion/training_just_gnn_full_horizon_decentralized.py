...
two_agent_gnn = GNNWithAgentPolicy(input_dim=4, hidden_dim=256, embedding_dim=4, horizon=H, action_dim=2)
# trainer = GNNBaselineTrainer(two_agent_gnn, device=device)
trainer = GNNBaselineTrainerDecentralized(two_agent_gnn, device=device)

# trainer.train([actions1, actions2], [attr1, attr2], n_gradient_steps=100000)
# trainer.save(path='trained_models/gnn_baseline_H_100_decentralized.pt')
...
for i in range(10):
    ...
    # sampled_traj = trainer.sample_trajectories(attr_t1, attr_t2)
    ...
    if not os.path.exists("figs/T10_2_gnn_H_100"):
        os.makedirs("figs/T10_2_gnn_H_100")
    plt.savefig("figs/T10_2_gnn_H_100/plot%s.png" % (i+10))