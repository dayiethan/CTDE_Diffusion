...
for epoch in tqdm(range(nb_epochs), desc="Training Progress"):
    ...
    # x_noised = x_noised * alphas_bar[t].unsqueeze(-1)

    x_noised[:,1:,:] = x0[:,1:,:]*torch.sqrt(alphas_bar[t]).unsqueeze(-1) + eps[:,1:,:]*torch.sqrt(1-alphas_bar[t]).unsqueeze(-1)

    pred = denoiser(x_noised, t.float())


    # Loss calculation with additional penalties
    initial_state_penalty = torch.linalg.vector_norm(x0[:, 0, :] - pred[:, 0, :])
    final_state_penalty = torch.linalg.vector_norm(x0[:, -1, :] - pred[:, -1, :])

    loss = torch.linalg.vector_norm(eps - pred) + 0.5 * (initial_state_penalty + final_state_penalty)
    # loss = torch.linalg.vector_norm(eps - pred)

    if loss.detach().item() < 3:
        print("Loss:",loss.detach().item())
        print("Epoch:",epoch)
        torch.save(denoiser.state_dict(), 'checkpoints/init_final_fixed/unet_diff_tran_cond_epoch'+str(epoch)+'.pth')
...
torch.save(denoiser.state_dict(), 'checkpoints/init_final_fixed/unet_diff_tran_cond_final.pth')