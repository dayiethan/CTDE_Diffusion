...
class PolicyPlayer:
    ...
    def reactive_mpc_plan(self, ode_model, obs, initial_states, pot, mean, std, segment_length=25, total_steps=250, n_implement=5):
        """
                ...
                if i == 0:
                    ...
                    if a1_store is not None:
                        for j in range(n_implement):
                            ...
                            # current_states[0] = state0[:3]
                            # current_states[1] = state1[:3]
    ...
    def get_demo(self, seed, cond_idx, extra, H=25, T=250):
        ...
        n_samples = 1
        # breakpoint()
        planned_trajs = self.reactive_mpc_plan(model, obs, [expert_data1[cond_idx, 0, :3], expert_data2[cond_idx, 0, :3]], pot[cond_idx], mean, std, segment_length=H, total_steps=T, n_implement=2)
