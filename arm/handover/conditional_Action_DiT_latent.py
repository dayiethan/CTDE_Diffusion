...
class Conditional_ODE():
    def __init__(self, env, encoder, attr_dim: list, sigma_data: list, sigma_min: float = 0.001, sigma_max: float = 50,
        ...
        all_params += list(self.latent_encoder.parameters())
    ...
    def train(self,
        ...
        for step in range(n_gradient_steps):
            ...
            for i in range(self.n_models):
                ...
                if i != 0:
                    embeddings = self.latent_encoder(latent[idx])
                    cond = torch.cat([attr, embeddings], dim=-1)
                    mask = (torch.rand_like(cond) > 0.2).float()
                    pred = self.D(x + eps, sigma, condition=cond, mask=mask, model_index=i)
                else:
