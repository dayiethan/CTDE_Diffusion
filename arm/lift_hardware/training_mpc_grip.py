...
with open("data/pot_states_1000.npy", "rb") as f:
    obs = np.load(f)
desired_index = np.array([0, 1, 2, 6])
obs_init1 = expert_data1[:, 0, desired_index]
obs_init2 = expert_data2[:, 0, desired_index]
obs_final1 = np.repeat(orig1[:, -1, desired_index], repeats=T, axis=0)
obs_final2 = np.repeat(orig2[:, -1, desired_index], repeats=T, axis=0)
...
# Training
end = "_lift_mpc_P200E1_1000T_grip"
...
# Sampling
def reactive_mpc_plan(ode_model, desired_index, initial_states, final_states, obs, segment_length=100, total_steps=1000, n_implement=1):
    """
    ...
    for seg in range(total_steps // n_implement):
        ...
        for i in range(n_agents):
            ...
            if seg == 0:
                ...
                new_state = seg_i[n_implement-1, desired_index]
            else:
                take = seg_i[1:n_implement+1, :]
                new_state = seg_i[n_implement, desired_index]
...
for i in range(10):
    cond_idx = i
    planned_trajs = reactive_mpc_plan(action_cond_ode, desired_index, [obs_init1[cond_idx], obs_init2[cond_idx]], [obs_final1[cond_idx], obs_final2[cond_idx]], obs[cond_idx], segment_length=H, total_steps=T, n_implement=2)
    planned_traj1 =  planned_trajs[0] * std + mean
    np.save("samples/P200E1_1000T_grip/planned_traj1_%s_new.npy" % cond_idx, planned_traj1)
    planned_traj2 = planned_trajs[1] * std + mean
    np.save("samples/P200E1_1000T_grip/planned_traj2_%s_new.npy" % cond_idx, planned_traj2)