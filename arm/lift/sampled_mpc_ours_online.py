...
class PolicyPlayer:
    ...
    def load_model(self, expert_data1, expert_data2, obs_init1, obs_init2, obs, state_dim = 7, action_dim = 7):
        ...
        action_cond_ode.load(extra="_lift_mpc_P25E1_crosscond_nofinalpos_fullstate_nolf_sitedata_newslower_rotvec_separatenorm_alternatemode_100")
    ...
    def get_demo(self, seed, cond_idx, H=25, T=700):
        ...
        # Loading
        expert_data = np.load("data/expert_actions_newslower_100_alternatemode.npy")
        ...
        with open("data/pot_start_newslower_100_alternatemode.npy", "rb") as f:
            obs = np.load(f)

        model = self.load_model(expert_data1, expert_data2, obs_init1, obs_init2, obs, state_dim = 7, action_dim = 7)

        planned_trajs = self.reactive_mpc_plan(model, [obs_init1[cond_idx], obs_init2[cond_idx]], obs[cond_idx], segment_length=H, total_steps=T*2, n_implement=20)
