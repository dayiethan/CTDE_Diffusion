...
class PolicyPlayer:
    ...
    def get_demo(self, seed, mode, sleeptime=0.03):
        ...
        import imageio
        video_path = "videos/two_arm_lift_mode%s_demo.mp4" % mode
...
if __name__ == "__main__":
    ...
    camera_heights=1080,               # << set height
    camera_widths=1920,
    )
    
    player = PolicyPlayer(env, render = False)
    rollout = player.get_demo(seed = 0, mode = 2)
    print("length of episode:", len(rollout["observations"]))
    rollout = player.get_demo(seed = 0, mode = 3)
    print("length of episode:", len(rollout["observations"]))
    # for i in range(200):   
    #     rollout = player.get_demo(seed = i*10, mode = 2)
    #     rollout['pot_start'] = [player.pot_handle0_pos, player.pot_handle1_pos]
    #     with open("rollouts/newslower_closegrip_orig/rollout_seed%s_mode2.pkl" % (i*10), "wb") as f:
    #         pkl.dump(rollout, f)
    #     rollout = player.get_demo(seed = i*10, mode = 3)
    #     rollout['pot_start'] = [player.pot_handle0_pos, player.pot_handle1_pos]
    #     with open("rollouts/newslower_closegrip_orig/rollout_seed%s_mode3.pkl" % (i*10), "wb") as f:
    #         pkl.dump(rollout, f)
