...
class PolicyPlayer:
    ...
    def load_model(self, expert_data1, expert_data2, obs_init1, obs_init2, obs, state_dim = 7, action_dim = 7):
        ...
        model1 = ImitationNet(input_size=input_size, hidden_size=256, output_size=7).to(device)
        model1.load_state_dict(torch.load(save_path1, map_location='cuda' if torch.cuda.is_available() else 'cpu'))
        model1.eval()

        model2 = ImitationNet(input_size=input_size, hidden_size=256, output_size=7).to(device)
    ...
    def reactive_mpc_plan(self, model1, model2, initial_states, pot, segment_length=25, total_steps=325, n_implement=2):
        """
    ...
    def get_demo(self, seed, cond_idx, H=25, T=700):
        ...
        model1, model2 = self.load_model(expert_data1, expert_data2, obs_init1, obs_init2, obs, state_dim = 7, action_dim = 7)

        planned_trajs = self.reactive_mpc_plan(model1, model2, [obs_init1[cond_idx], obs_init2[cond_idx]], obs[cond_idx], segment_length=H, total_steps=T*2, n_implement=10)
