...
# Distance at the *start* of each MPC window (CTDE "current time")
dist_seq = np.linalg.norm(expert_data1[:, 0, :3] - expert_data2[:, 0, :3], axis=1)   # (N,)
dist_mean = dist_seq.mean().astype(np.float32)
np.save("data/mean_100_separatenorm_dist_eefdist.npy", dist_mean)
dist_std  = dist_seq.std().astype(np.float32) + 1e-6
np.save("data/std_100_separatenorm_dist_eefdist.npy", dist_std)
dist0 = ((dist_seq - dist_mean) / dist_std).reshape(-1, 1).astype(np.float32)       # (N,1)

# Compute mean and standard deviation
mean_arm1 = np.mean(expert_data1, axis=(0,1))
np.save("data/mean_100_separatenorm_dist_arm1.npy", mean_arm1)
std_arm1 = np.std(expert_data1, axis=(0,1))
np.save("data/std_100_separatenorm_dist_arm1.npy", std_arm1)
mean_arm2 = np.mean(expert_data2, axis=(0,1))
np.save("data/mean_100_separatenorm_dist_arm2.npy", mean_arm2)
std_arm2 = np.std(expert_data2, axis=(0,1))
np.save("data/std_100_separatenorm_dist_arm2.npy", std_arm2)

mean_pot1 = np.mean(pot1, axis=(0, 1))
mean_pot2 = np.mean(pot2, axis=(0, 1))
np.save("data/mean_100_separatenorm_dist_pot1.npy", mean_pot1)
np.save("data/mean_100_separatenorm_dist_pot2.npy", mean_pot2)
std_pot1 = np.std(pot1, axis=(0, 1))
std_pot2 = np.std(pot2, axis=(0, 1))
np.save("data/std_100_separatenorm_dist_pot1.npy", std_pot1)
np.save("data/std_100_separatenorm_dist_pot2.npy", std_pot2)
...
obs2 = np.hstack([obs_init2, obs_init1, pot2[:, 0, :], dist0])
...
# Training
end="_lift_mpc_P25E1_crosscond_nofinalpos_fullstate_lf_sitedata_grippauseshort_addpoints_rotvec_separatenorm_dist_currpotpos"
