...
class PolicyPlayer:
    ...
    def load_model(self, expert_data1, expert_data2, obs_init1, obs_init2, pot_all, state_dim=7, action_dim=7):
        model_size = {"d_model": 256, "n_heads": 4, "depth": 3}
        H = 25  # horizon used during training
        # Infer pot feature dimension from the array you pass in (N, T, D_pot) or (T, D_pot)
        pot_dim = int(pot_all.shape[-1])
        ...
        # IMPORTANT: load the checkpoint trained with the distance feature
        action_cond_ode.load(
            extra="_lift_mpc_P25E1_crosscond_nofinalpos_fullstate_lf_sitedata_grippauseshort_addpoints_rotvec_separatenorm_dist"
        )
    ...
    def reactive_mpc_plan(self, ode_model, initial_states, pot,
        ...
        for seg in range(total_steps // n_implement):
            ...
            for i in range(len(base_states)):
                if i == 0:
                    # Leader: [self_norm, pot_raw]
                    cond_parts = [base_states[i], pot_vec]
                else:
                    # Follower: [self_norm, leader_norm, pot_raw, dist_norm]
                    cond_parts = [base_states[i], base_states[0], pot_vec, dist_n]
    ...
    def get_demo(self, seed, cond_idx, H=25, T=325):
        ...
        planned_trajs = self.reactive_mpc_plan(model, [obs_init1[cond_idx], obs_init2[cond_idx]], obs[cond_idx], segment_length=H, total_steps=T*2, n_implement=8)
