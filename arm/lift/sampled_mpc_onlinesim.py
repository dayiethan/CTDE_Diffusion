...
class PolicyPlayer:
    ...
    def check_arrived(self, pos1, rotm1, grip1, pos2, rotm2, grip2, threshold = 0.35):
        pos_diff = pos1 - pos2
        ...
        # print("pos_diff:", 0.5 * np.linalg.norm(pos_diff)**2)
        # print("rotm_diff:", np.trace(np.eye(3) - rotm_diff))
        # print("grip_diff:", 0.5 * grip_diff**2)
    ...
    def execute_mpc_online(self, seed, ode_model, curr_pot, segment_length=25, total_steps=250, n_implement=2):
        """
        ...
        for env_step_count in range(total_steps):
            if env_step_count % n_implement == 0:  # Time to replan
                r0_current_planning_state, r1_current_planning_state = self._extract_planning_states_from_env_obs(obs)

                # Plan for robot 0
                cond0_list = [r0_current_planning_state[:3], curr_pot]
                ...
                # Plan for robot 1
                cond1_list = [r1_current_planning_state[:3], planned_actions_segment_r0[n_implement, :3], curr_pot]
            ...
            obs, reward, done, info = self.env.step(combined_action_to_env)
