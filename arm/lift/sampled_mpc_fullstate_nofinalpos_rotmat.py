...
class PolicyPlayer:
    ...
    def load_model(self, state_dim = 10, action_dim = 10):
        ...
        action_cond_ode.load(extra="_lift_mpc_P25E1_crosscond_nofinalpos_fullstate_nolf_sitedata_grippause")
    ...
    def get_demo(self, seed, cond_idx, H=25, T=250):
        ...
        model = self.load_model(state_dim = 10, action_dim = 10)

        with open("data/pot_states_rot6d_site_grippause_20.npy", "rb") as f:
            obs = np.load(f)

        planned_trajs = self.reactive_mpc_plan(model, [obs_init1[cond_idx], obs_init2[cond_idx]], obs[cond_idx], segment_length=H, total_steps=T, n_implement=10)
        planned_traj1 =  planned_trajs[0] * std + mean
        # np.save("samples/mpc_P25E10/mpc_traj1.npy", planned_traj1)
        planned_traj2 = planned_trajs[1] * std + mean
        # np.save("samples/mpc_P25E10/mpc_traj2.npy", planned_traj2)

        # planned_traj1 = np.load("samples/mpc_P25E10/mpc_traj1.npy")
        # planned_traj2 = np.load("samples/mpc_P25E10/mpc_traj2.npy")

        traj1_rot6d = planned_traj1[:, 3:9]
        traj2_rot6d = planned_traj2[:, 3:9]
        traj1_rotvec = np.array([rot6d_to_rotvec(rot6d) for rot6d in traj1_rot6d])
        traj2_rotvec = np.array([rot6d_to_rotvec(rot6d) for rot6d in traj2_rot6d])
        traj1_fin = np.hstack([planned_traj1[:, :3], traj1_rotvec, planned_traj1[:, 9:]])
        traj2_fin = np.hstack([planned_traj2[:, :3], traj2_rotvec, planned_traj2[:, 9:]])

        breakpoint()

        # Run the sampled trajectory in the environment
        for i in range(len(traj1_fin)):
            action = np.hstack([traj1_fin[i], traj2_fin[i]])
