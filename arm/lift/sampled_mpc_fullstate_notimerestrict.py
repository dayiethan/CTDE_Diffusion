...
class PolicyPlayer:
    ...
    def load_model(self, type = "rotvec", state_dim = 7, action_dim = 7):
        ...
        action_cond_ode.load(extra="_lift_mpc_P25E1_crosscond_finalpos_fullstate_nolf")

        return action_cond_ode
    
    
    def reactive_mpc_plan(self, ode_model, initial_states, final_states, obs, segment_length=25, max_steps=350, n_implement=5):
        full_traj = []
        current_states = initial_states.copy()

        for seg in range(max_steps // n_implement):
            segments = []
    ...
    def get_demo(self, seed, cond_idx, H=25, T=250):
        ...
        planned_trajs = self.reactive_mpc_plan(model, [obs_init1[cond_idx], obs_init2[cond_idx]], [obs_final1[cond_idx], obs_final2[cond_idx]], obs[cond_idx], segment_length=H, max_steps=T + 100, n_implement=1)
