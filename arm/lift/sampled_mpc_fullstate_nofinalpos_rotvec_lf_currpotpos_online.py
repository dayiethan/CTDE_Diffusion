...
class PolicyPlayer:
    ...
    def load_model(self, expert_data1, expert_data2, obs_init1, obs_init2, pot1, pot2, state_dim = 7, action_dim = 7):
        ...
        action_cond_ode.load(extra="_lift_mpc_P25E1_crosscond_nofinalpos_fullstate_lf_sitedata_grippauseshort_addpoints_currpotpos_rotvec")
    ...
    def reactive_mpc_plan(self, ode_model, initial_states, pots, segment_length=25, total_steps=325, n_implement=2):
        """
        ...
        current_pot1 = pots[0].copy()
        current_pot2 = pots[1].copy()
        ...
        for seg in range(total_steps // n_implement):
            ...
            for i in range(len(base_states)):
                ...
                if i == 0:
                    cond = [base_states[i], current_pot1]
                else:
                    cond = [base_states[i]] + [base_states[j] for j in range(len(base_states)) if j!=i] + [current_pot2]
                cond = np.hstack(cond)
            ...
            for t in range(n_implement):
                ...
                action = np.hstack([action1, action2])  # shape (14,)
                obs_env, reward, done, info = self.env.step(action)
                if self.render:
                    self.env.render()

                current_pot1, current_pot2 = self.get_pot_state_local()  # (6,)
    ...
    def get_demo(self, seed, cond_idx, H=25, T=325):
        ...
        expert_data = np.load("data/expert_actions_rotvec_site_grippauseshort_addpoints_currpotpos_20.npy")
        pot1_raw = np.load("data/pot_states1_rotvec_site_grippauseshort_addpoints_currpotpos_20.npy")
        pot2_raw = np.load("data/pot_states2_rotvec_site_grippauseshort_addpoints_currpotpos_20.npy")
        ...
        try:
            self.mean_arm1 = np.load("data/mean_arm1.npy")
            self.mean_arm2 = np.load("data/mean_arm2.npy")
            self.std_arm1 = np.load("data/std_arm1.npy")
            self.std_arm2 = np.load("data/std_arm2.npy")
            self.mean_pot1 = np.load("data/mean_pot1.npy")
            self.mean_pot2 = np.load("data/mean_pot2.npy")
            self.std_pot1 = np.load("data/std_pot1.npy")
            self.std_pot2 = np.load("data/std_pot2.npy")
        except FileNotFoundError:
            self.mean_arm1 = np.mean(expert_data1, axis=(0, 1))
            self.mean_arm2 = np.mean(expert_data2, axis=(0, 1))
            np.save("data/mean_arm1.npy", self.mean_arm1)
            np.save("data/mean_arm2.npy", self.mean_arm2)
            self.std_arm1 = np.std(expert_data1, axis=(0, 1))
            self.std_arm2 = np.std(expert_data2, axis=(0, 1))
            np.save("data/std_arm1.npy", self.std_arm1)
            np.save("data/std_arm2.npy", self.std_arm2)
            self.mean_pot1 = np.mean(pot1, axis=(0, 1))
            self.mean_pot2 = np.mean(pot2, axis=(0, 1))
            np.save("data/mean_pot1.npy", self.mean_pot1)
            np.save("data/mean_pot2.npy", self.mean_pot2)
            self.std_pot1 = np.std(pot1, axis=(0, 1))
            self.std_pot2 = np.std(pot2, axis=(0, 1))
            np.save("data/std_pot1.npy", self.std_pot1)
            np.save("data/std_pot2.npy", self.std_pot2)
        ...
        pot_init1 = pot1[:, 0, :]
        pot_init2 = pot2[:, 0, :]
        obs1 = np.hstack([obs_init1, pot_init1])
        obs2 = np.hstack([obs_init2, obs_init1, pot_init2])
        obs1 = torch.FloatTensor(obs1).to(device)
        obs2 = torch.FloatTensor(obs2).to(device)
        attr1 = obs1
        attr2 = obs2
        ...
        # pot_raw = np.load("data/pot_states_rotvec_site_grippauseshort_addpoints_20.npy")
        # expert_data1 = expert_data[:, :, :7]
        # expert_data2 = expert_data[:, :, 7:14]
        # expert_data1 = create_mpc_dataset(expert_data1, planning_horizon=H)
        # expert_data2 = create_mpc_dataset(expert_data2, planning_horizon=H)
        # pot_seq = create_mpc_dataset(pot_raw, planning_horizon=H)
        # combined_data = np.concatenate((expert_data1, expert_data2), axis=0)
        # self.mean = np.mean(combined_data, axis=(0,1))
        # self.std = np.std(combined_data, axis=(0,1))
        # self.pot_mean = np.mean(pot_seq, axis=(0, 1))
        # self.pot_std  = np.std(pot_seq, axis=(0, 1))

        # # Normalize data
        # expert_data1 = (expert_data1 - self.mean) / self.std
        # expert_data2 = (expert_data2 - self.mean) / self.std
        # pot = (pot_seq - self.pot_mean) / self.pot_std
        ...
        planned_trajs = self.reactive_mpc_plan(model, [obs_init1[cond_idx], obs_init2[cond_idx]], [pot_init1[cond_idx], pot_init2[cond_idx]], segment_length=H, total_steps=T*2, n_implement=5)
