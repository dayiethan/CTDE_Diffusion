...
class PolicyPlayer:
    ...
    def setup_waypoints(self):
        ...
        if self.handle_length < 0.12:
            ...
            robot1_pos_xd = 1.45 - robot0_pos_xd - self.handle_length * 0.5 + np.random.uniform(-0.02, 0.00)
        #wp0
    ...
    # def convert_action_robot(self, current_pos, current_rotm, goal_pos, goal_rotm, robot_gripper):
    #     action = np.zeros(int(self.n_action/2))
    #     # Compute delta position
    #     delta_pos = goal_pos - current_pos
    #     # Compute delta rotation as axis-angle (relative rotation)
    #     current_rot = R.from_matrix(current_rotm)
    #     desired_rot = R.from_matrix(goal_rotm)
    #     delta_rot = (current_rot.inv() * desired_rot).as_rotvec()
    ...
    def convert_action_robot(self, current_pos, current_rotm, goal_pos, goal_rotm, robot_gripper):
        ...
        # Compute delta position
        delta_pos = goal_pos - current_pos
        pos_scale = 0.5  # Scale factor for position
        delta_pos *= pos_scale
        ...
        desired_rot = R.from_matrix(goal_rotm)
        delta_rot = (current_rot.inv() * desired_rot).as_rotvec()
        rot_scale = 0.8  # Scale factor for rotation
        delta_rot *= rot_scale
    ...
    def get_demo(self, seed):
        ...
        # stage 1: robot0 move to wp0, robot1 moves to wp0 and then wp1
        for i in range(max_step_move):
            ...
            if not robot0_arrived:
                ...
                action0 = self.convert_action_robot(robot0_pos, robot0_rotm,
                                                    goal_pos0, goal_rotm0,
                                                    self.waypoints_robot0[0]["gripper"])
                robot0_arrived = self.check_arrived(robot0_pos, robot0_rotm,
                                                    goal_pos0, goal_rotm0,
                                                    threshold=0.1)
            ...
            if not robot1_arrived_0:
                ...
                action1 = self.convert_action_robot(robot1_pos, robot1_rotm,
                                                    goal_pos1, goal_rotm1,
                                                    self.waypoints_robot1[0]["gripper"])
                robot1_arrived_0 = self.check_arrived(robot1_pos, robot1_rotm,
                                                    goal_pos1, goal_rotm1,
                                                    threshold=0.2)
                ...
                action1 = self.convert_action_robot(robot1_pos, robot1_rotm,
                                                    goal_pos1, goal_rotm1,
                                                    self.waypoints_robot1[1]["gripper"])
                robot1_arrived_1 = self.check_arrived(robot1_pos, robot1_rotm,
                                                    goal_pos1, goal_rotm1,
                                                    threshold=0.05)
        ...
        for i in range(max_step_move):
            ...
            if not robot0_arrived:
                ...
                # Compute delta command for robot0 using current pose and goal
                action0 = self.convert_action_robot(robot0_pos, robot0_rotm,
                                                    goal_pos0, goal_rotm0,
                                                    self.waypoints_robot0[1]["gripper"])
                robot0_arrived = self.check_arrived(robot0_pos, robot0_rotm,
                                                    goal_pos0, goal_rotm0,
                                                    threshold=0.05)
            ...
            if not robot1_arrived:
                ...
                # Compute delta command for robot1 using current pose and goal
                action1 = self.convert_action_robot(robot1_pos, robot1_rotm,
                                                    goal_pos1, goal_rotm1,
                                                    self.waypoints_robot1[2]["gripper"])
                robot1_arrived = self.check_arrived(robot1_pos, robot1_rotm,
                                                    goal_pos1, goal_rotm1,
                                                    threshold=0.05)
    ...
    def test(self):
        ...
        for i in range(self.max_steps):
            action0 = self.convert_action_robot(goal_pos0, goal_rotm0, 1) #(7,)
            action1 = self.convert_action_robot(goal_pos1, goal_rotm1, 1) #(7,)
...
if __name__ == "__main__":
    ...
    # controller_config = load_composite_controller_config(robot="Kinova3", controller="BASIC")
    controller_config1 = load_composite_controller_config(robot="Kinova3", controller="kinova_osc_pose1.json")
    controller_config2 = load_composite_controller_config(robot="Kinova3", controller="kinova_osc_pose2.json")
    # print(controller_config.keys())
    # print(controller_config["body_parts"])
    # import sys
    # sys.exit()
    ...
    controller_configs=[controller_config1, controller_config2],
    has_renderer=True,
