...
for epoch in tqdm(range(nb_epochs), desc="Training Progress"):
    ...
    integers1 = torch.randint(0, trajectory1.shape[0], (batch_size,))
    integers2 = torch.randint(0, trajectory2.shape[0], (batch_size,))
    
    # choose the corresponding trajectory
    x01 = torch.tensor(trajectory1[integers1,:,2:4]).float()
    x02 = torch.tensor(trajectory2[integers2,:,2:4]).float()
...
torch.save(denoiser1.state_dict(), 'checkpoints_shared/unet1_diff_tran_final.pth')
torch.save(denoiser2.state_dict(), 'checkpoints_shared/unet2_diff_tran_final.pth')