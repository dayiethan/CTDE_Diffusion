...
denoiser.load_state_dict(torch.load("checkpoints/unet_diff_tran_cond_epoch1999.pth"))
...
def compute_action_diff(alphas_bar, alphas, betas, denoiser, initial_state, final_state):
    ...
    # u0 = torch.randn((1, 100, 6)) * torch.sqrt(0.1 * (1 - alpha_bar)) + torch.sqrt(alpha_bar)
...
def main():
    ...
    noise_level_x = 0.0
    noise_level_y = 10.0
    x0 = np.array([0.0,0.0,0.0,20.0,0.0,np.pi]) + np.array([noise_level_x*(2*np.random.rand()-1), noise_level_y*(2*np.random.rand()-1), 0, 
                                                            noise_level_x*(2*np.random.rand()-1), noise_level_y*(2*np.random.rand()-1), 0])
    xf = np.array([20.0,0.0,np.pi,0.0,0.0,0.0]) + np.array([noise_level_x*(2*np.random.rand()-1), noise_level_y*(2*np.random.rand()-1), 0, 
                                                            noise_level_x*(2*np.random.rand()-1), noise_level_y*(2*np.random.rand()-1), 0])
    x0 = np.array([0.0,0.0,0.0,20.0,0.0,np.pi])
    xf = np.array([20.0,0.0,np.pi,0.0,0.0,0.0])
    ...
    traj[0,4:] = x0
    traj[-1,4:] = xf
    print('Trajectory Array Size: {0}'.format(traj.shape))
    print("Trajectory Array Init: ", traj[0])
    print("Trajectory Array Final: ", traj[-1])

    max_traj_array = np.loadtxt("data/max_traj_array_rand.csv", delimiter=",")
    print("Max Trajectory Array Size: {0}".format(np.shape(max_traj_array)))
    print("Max Trajectory Array: ", max_traj_array)

    action_normalization_arr = max_traj_array[0:4]
    state_normalization_arr = max_traj_array[4:]

    # Normalize initial and final states
    x0_normalized = x0 / state_normalization_arr
    xf_normalized = xf / state_normalization_arr

    # Generate trajectory
    u_out = compute_action_diff(alphas_bar, alphas, betas, denoiser, x0_normalized, xf_normalized)
    ...
    print("Normalized Trajectory Array Init: ", traj[0])
    print("Normalized Trajectory Array Final: ", traj[-1])
    ...
    fig = plt.figure()
    ax = plt.axes(xlim=(-1.5, 20.5), ylim=(-20, 20))
    ...
    anim.save('figs/test_trajectory_animation_init_final_fixed.gif')
    plt.show()
