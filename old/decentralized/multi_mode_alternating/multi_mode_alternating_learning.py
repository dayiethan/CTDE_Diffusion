...
for epoch in range(num_epochs):
    ...
    x02 = torch.tensor(expert_data_rev[integers2]).float()
    ...
    x01 = torch.tensor(expert_data[integers1]).float()
    eps1 = torch.randn_like(x01)
    x_noised1  = x01
    ...
    if (epoch+1)%1000 == 0:
        torch.save(denoiser1.state_dict(), 'checkpoints_alternating/unet1_diff_tran_epoch'+str(epoch)+'.pth')
        torch.save(denoiser2.state_dict(), 'checkpoints_alternating/unet2_diff_tran_epoch'+str(epoch)+'.pth')
