...
# 2) Hyperparameters
state_dim   = 2       # e.g. [x, y, goal_x, goal_y]
...
def build_sa(expert):
    ...
    for traj in expert:
        for t in range(len(traj) - 1):
            s = traj[t]        # current state only
            a = traj[t + 1]    # next state (target)
...
# 8) Save models
torch.save(G1.state_dict(), "trained_models/magail_ctde/G1_nofinalpos.pth")
torch.save(G2.state_dict(), "trained_models/magail_ctde/G2_nofinalpos.pth")
torch.save(G3.state_dict(), "trained_models/magail_ctde/G3_nofinalpos.pth")
torch.save(D.state_dict(),  "trained_models/magail_ctde/D_nofinalpos.pth")
print("CTDE MAGAIL training complete; models saved under trained_models/magail_ctde/")


# 9) Sampling  — current-state only (no goal conditioning)
def sample_trajectory(G, init_pos, horizon):
    pos = init_pos.copy()
    traj = [pos.copy()]
    for _ in range(horizon - 1):
        s = pos  # current state only
...
for i in range(num_samples):
    ...
    init1 = expert_data1[idx, 0]
    init2 = expert_data2[idx, 0]
    init3 = expert_data3[idx, 0]

    traj1 = sample_trajectory(G1, init1, T)
    np.save(f"sampled_trajs/magail_ctde_nofinalpos/vary_init/traj1_{i}.npy", traj1)
    traj2 = sample_trajectory(G2, init2, T)
    np.save(f"sampled_trajs/magail_ctde_nofinalpos/vary_init/traj2_{i}.npy", traj2)
    traj3 = sample_trajectory(G3, init3, T)
    np.save(f"sampled_trajs/magail_ctde_nofinalpos/vary_init/traj3_{i}.npy", traj3)
...
# Overlay start points only (goals removed)
...
plt.title("MAGAIL-Generated Trajectories (current-state conditioned)")
plt.xlabel("x")
plt.ylabel("y")
plt.legend()
plt.axis('equal')
...
# --- pick one fixed context (start only) ---------------------
...
N = 100
rollouts1 = [sample_trajectory(G1, init1, T) for _ in range(N)]
rollouts2 = [sample_trajectory(G2, init2, T) for _ in range(N)]
rollouts3 = [sample_trajectory(G3, init3, T) for _ in range(N)]
for i in range(N):
    np.save(f"sampled_trajs/magail_ctde_nofinalpos/static_init/traj1_{i}.npy", rollouts1[i])
    np.save(f"sampled_trajs/magail_ctde_nofinalpos/static_init/traj2_{i}.npy", rollouts2[i])
    np.save(f"sampled_trajs/magail_ctde_nofinalpos/static_init/traj3_{i}.npy", rollouts3[i])
...
plt.title("Mode Collapse Check (fixed start)")
