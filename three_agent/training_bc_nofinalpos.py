...
for i in range(n_samples):
    ...
    traj2 = [init2]
    traj3 = [init3]
    s1 = torch.from_numpy(np.hstack([init1, init2, init3])).float().to(device).unsqueeze(0)
    s2 = torch.from_numpy(np.hstack([init2, init1, init3])).float().to(device).unsqueeze(0)
    s3 = torch.from_numpy(np.hstack([init3, init1, init2])).float().to(device).unsqueeze(0)
