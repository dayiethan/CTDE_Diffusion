...
# Build BC training data: (current_state) -> next_state
def build_bc_data(expert_data):
    X, Y = [], []
    for traj in expert_data:
        for t in range(len(traj) - 1):
            current = traj[t]
            X.append(np.hstack([current]))  # [x_t, x_0]
            Y.append(traj[t + 1])                 # x_{t+1}
    return np.array(X, dtype=np.float32), np.array(Y, dtype=np.float32)


X1, Y1 = build_bc_data(expert_data1)
X2, Y2 = build_bc_data(expert_data2)
X3, Y3 = build_bc_data(expert_data3)
...
input_size = X1.shape[1]  # should be 4
model1 = ImitationNet(input_size, 64, 2).to(device)
model2 = ImitationNet(input_size, 64, 2).to(device)
model3 = ImitationNet(input_size, 64, 2).to(device)
...
# Save trained models
save_path1 = "trained_models/bc/model1_nofinalpos.pth"
save_path2 = "trained_models/bc/model2_nofinalpos.pth"
save_path3 = "trained_models/bc/model3_nofinalpos.pth"

torch.save(model1.state_dict(), save_path1)
torch.save(model2.state_dict(), save_path2)
torch.save(model3.state_dict(), save_path3)

model1 = ImitationNet(input_size=2, hidden_size=64, output_size=2).to(device)
model1.load_state_dict(torch.load(save_path1, map_location='cuda' if torch.cuda.is_available() else 'cpu'))
model1.eval()

model2 = ImitationNet(input_size=2, hidden_size=64, output_size=2).to(device)
model2.load_state_dict(torch.load(save_path2, map_location='cuda' if torch.cuda.is_available() else 'cpu'))
model2.eval()

model3 = ImitationNet(input_size=2, hidden_size=64, output_size=2).to(device)
...
for i in range(n_samples):
    # choose initial states (add noise if you want)
    init1 = init1_list[i]
    init2 = init2_list[i]
    init3 = init3_list[i]
    ...
    # network input is [current_state, initial_state]
    s1 = torch.from_numpy(np.hstack([init1])).float().to(device).unsqueeze(0)
    s2 = torch.from_numpy(np.hstack([init2])).float().to(device).unsqueeze(0)
    s3 = torch.from_numpy(np.hstack([init3])).float().to(device).unsqueeze(0)
    ...
    for _ in range(traj_length - 1):
        ...
        # update "current", keep "initial" fixed
        s1 = torch.from_numpy(np.hstack([next1])).float().to(device).unsqueeze(0)
        s2 = torch.from_numpy(np.hstack([next2])).float().to(device).unsqueeze(0)
        s3 = torch.from_numpy(np.hstack([next3])).float().to(device).unsqueeze(0)

    generated[1].append(np.array(traj1)); np.save(f"sampled_trajs/bc_nofinalpos/traj1_{i}.npy", np.array(traj1))
    generated[2].append(np.array(traj2)); np.save(f"sampled_trajs/bc_nofinalpos/traj2_{i}.npy", np.array(traj2))
    generated[3].append(np.array(traj3)); np.save(f"sampled_trajs/bc_nofinalpos/traj3_{i}.npy", np.array(traj3))
...
# plot start markers only (optional)
for c, init in zip(['b', 'g', 'r'], [initial_point1, initial_point2, initial_point3]):
    plt.scatter(init[0], init[1], c=c)

plt.xlabel('X'); plt.ylabel('Y')
plt.title('BC Sampled Trajectories (Initial-state conditioned)')
plt.grid(True)
plt.show()
