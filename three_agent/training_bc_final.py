...
# Build BC training data: (current_state, final_state) -> next_state
def build_bc_data(expert_data, other1, other2):
    X, Y = [], []
    for i in range(len(expert_data)):
        ...
        for t in range(len(traj)-1):
            X.append(np.hstack([traj[t], other_traj1[t], other_traj2[t]]))
            Y.append(traj[t+1])
...
input_size = X1.shape[1]  # should be 6
...
print(f"Total parameters: {sum(p.numel() for p in model1.parameters()) + sum(p.numel() for p in model2.parameters()) + sum(p.numel() for p in model3.parameters())}")
...
# Save trained models
save_path1 = "trained_models/bc/bc1_nofinalpos_big.pth"
save_path2 = "trained_models/bc/bc2_nofinalpos_big.pth"
save_path3 = "trained_models/bc/bc3_nofinalpos_big.pth"

# torch.save(model1.state_dict(), save_path1)
# torch.save(model2.state_dict(), save_path2)
# torch.save(model3.state_dict(), save_path3)

model1 = ImitationNet(input_size=6, hidden_size=2000, output_size=2).to(device)
model1.load_state_dict(torch.load(save_path1, map_location='cuda' if torch.cuda.is_available() else 'cpu'))
model1.eval()

model2 = ImitationNet(input_size=6, hidden_size=2000, output_size=2).to(device)
model2.load_state_dict(torch.load(save_path2, map_location='cuda' if torch.cuda.is_available() else 'cpu'))
model2.eval()

model3 = ImitationNet(input_size=6, hidden_size=2000, output_size=2).to(device)
...
for s in range(10):
    ...
    torch.manual_seed(seed)
    random.seed(seed)
    path = f"sampled_trajs/bc_seed{seed}"
    ...
    for i in range(n_samples):
        ...
        traj2 = [initial2]
        traj3 = [initial3]
        s1 = torch.from_numpy(np.hstack([initial1, initial2, initial3])).float().to(device).unsqueeze(0)
        s2 = torch.from_numpy(np.hstack([initial2, initial1, initial3])).float().to(device).unsqueeze(0)
        s3 = torch.from_numpy(np.hstack([initial3, initial1, initial2])).float().to(device).unsqueeze(0)
        ...
        for _ in range(traj_length-1):
            ...
            traj2.append(next2)
            traj3.append(next3)
            s1 = torch.from_numpy(np.hstack([next1, next2, next3])).float().to(device).unsqueeze(0)
            s2 = torch.from_numpy(np.hstack([next2, next1, next3])).float().to(device).unsqueeze(0)
            s3 = torch.from_numpy(np.hstack([next3, next1, next2])).float().to(device).unsqueeze(0)
