...
D3 = DiscNet().to(device)

optG = optim.Adam(list(G1.parameters()) + list(G2.parameters()) + list(G3.parameters()), lr=lr_G)
optD = optim.Adam(list(D1.parameters()) + list(D2.parameters()) + list(D3.parameters()), lr=lr_D)
...
tS3 = torch.tensor(S3, dtype=torch.float32)
tA3 = torch.tensor(A3, dtype=torch.float32)

loader1 = DataLoader(TensorDataset(tS1,tA1),
                     batch_size=batch_size, shuffle=True, drop_last=True)
loader2 = DataLoader(TensorDataset(tS2,tA2),
                     batch_size=batch_size, shuffle=True, drop_last=True)
loader3 = DataLoader(TensorDataset(tS3,tA3),
                     batch_size=batch_size, shuffle=True, drop_last=True)

# 7) Training loop
for epoch in range(1, n_epochs+1):
    G1.train(); G2.train(); G3.train(); D1.train(); D2.train(); D3.train()
    lossD_sum = 0.0
    lossG_sum = 0.0
    ...
    for (x1, a1), (x2, a2), (x3, a3) in zip(loader1, loader2, loader3):
        ...
        with torch.no_grad():
            fake3 = G3(x3)
        real_logit3 = D3(x3, a3)
        fake_logit3 = D3(x3, fake3)
        lossD3 = 0.5*(bce_logits(real_logit3, torch.ones_like(real_logit3)) +
                      bce_logits(fake_logit3, torch.zeros_like(fake_logit3)))
        ...
        fake3 = G3(x3)
        lossG3 = bce_logits(D3(x3, fake3), torch.ones_like(fake3[:,0]))
...
torch.save(D3.state_dict(), "trained_models/magail/D3.pth")
