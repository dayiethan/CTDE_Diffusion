...
for i in range(200):
    denoiser1 = DiT1d(x_dim=2, attr_dim=1, d_model=64, n_heads=4, depth=3, dropout=0.1)
    denoiser2 = DiT1d(x_dim=2, attr_dim=1, d_model=64, n_heads=4, depth=3, dropout=0.1)
    denoiser1.load_state_dict(torch.load("checkpoints_0.75/unet1_diff_tran_epoch2999.pth"))
    denoiser2.load_state_dict(torch.load("checkpoints_0.75/unet2_diff_tran_epoch2999.pth"))
    ...
    plt.ylabel('Y')
    plt.grid(True)
    plt.savefig('figs_0.75/expert_vs_generated_trajectories%s.png' % i)
    # plt.show()
