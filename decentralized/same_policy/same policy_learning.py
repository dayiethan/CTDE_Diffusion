...
initial_point_fwd = np.array([0.0, 0.0])
final_point_fwd = np.array([20.0, 0.0])
...
all_points_rev = list(reversed(all_points_fwd))

all_points = all_points_fwd + all_points_rev
...
for traj in expert_data:
    for i in range(len(traj) - 1):
        X_train_fwd.append(np.hstack([traj[i], final_point_fwd]))  # Current state + goal
        Y_train_fwd.append(traj[i + 1])  # Next state

X_train_fwd = torch.tensor(np.array(X_train_fwd), dtype=torch.float32)  # Shape: (N, 4)
Y_train_fwd = torch.tensor(np.array(Y_train_fwd), dtype=torch.float32)  # Shape: (N, 2)
...
for epoch in range(num_epochs):
    ...
    if (epoch+1)%1000 == 0:
        torch.save(denoiser1.state_dict(), 'checkpoints_multimode/unet_diff_tran_epoch'+str(epoch)+'.pth')
